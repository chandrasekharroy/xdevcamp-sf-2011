...
//add event handler to add to bag button via javascript

if (document.getElementById("frmEsBottom") != null)
{
	var addToBagBtn = document.getElementById("frmEsBottom").getElementsByTagName('a');
	addToBagBtn.addListener(addToBagBtn, "click", function()
	{
		var bagTimer = setTimeout(addToBagCloseES,1000);
		clearTimeout(bagTimer);
	});
}
function addToBagCloseES()
{
	hideExpressShop();
	hideLightbox();
}
  ...
  if( typeof( window.pageYOffset ) == 'number' ) {
    ...
    if (isIE)
    {
      ...
      scrOfX = document.documentElement.scrollLeft;
		...
		document.getElementById("expressShop").style.left = leftMarginBegins + "px";
    if (isIE)
    {//IE specific
      document.getElementById("expressShop").style.top = (scrOfY + 90) + "px";
    }
    else
    {//Everyone else
      document.getElementById("expressShop").style.top = topMarginBegins + "px";
    }
  }
}
...
function showProductPopup_sizeChart(showThisProdID, cp, currentCategoryId, currentProduct)
{

	createXMLHttpRequest();
	//reset variables
	resetVars();
	var queryString2 = "../include/productDetails.jsp?productId="+showThisProdID+"&currentCp="+cp+"&hasSizeChart=true&currentCategoryId="+currentCategoryId+"&currentProduct="+currentProduct;
...
function showProductPopup(showThisProdID, cp, currentCategoryId, currentProduct)
{

	createXMLHttpRequest();
	//reset variables
	resetVars();
	var queryString2 = "../include/productDetails.jsp?productId="+showThisProdID+"&currentCp="+cp+"&currentCategoryId="+currentCategoryId+"&currentProduct="+currentProduct;
...
{
	window.box = new Df.Modal()
	box.pars.animateHolder = {
		time: 150,
		pause: 60,
		opacity:.5,
	 selectors:['opacity']
	...
	box.pars.minWidth = pageWidth;
	if(pageHeight && pageHeight >0)
		box.pars.minHeight = pageHeight;
	else
		box.pars.minHeight = 800;
...
// Set the contents of the expressShop div to blank.
	...
	{ 
		...
		{
			box.set();
			box.setContentString('<div id="expressShop">' + xmlHttp.responseText + '</div>');
			box.show();
			execJS($("expressShop"));
			document.getElementById("expressShop").style.display = "";
		}
...
//	centerDiv();
	...
	if (document.getElementById("expressShop") != null)
		document.getElementById("expressShop").style.zIndex= "103";
}
function showExpressShop1()
{
	// Set the contents of the expressShop div to blank.
...
if (toggleTabOn_!= null)
	toggleTabOn_.style.display = 'block';
if (toggleTabOff_!= null)
	toggleTabOff_.style.display = 'none';
if (toggleTabOff2_!= null)
...
function setSwatchSize(size)
{
    ...
    if(size == 0)
    {
		$('esProductAvailColors').style.display="none";
    }
}
...
{
	//alert("miniCartLen : "+miniCartLen+"	newCartProducts.length : "+newCartProducts.length);
	var i = (newCartProducts.length > miniCartLen) ? miniCartLen - 1:newCartProducts.length - 1;
	//alert("i : "+i);
	//var i = newCartProducts.length - 1;
	for (i; i >=0 ; i--)
	{
		var cartDiv3 = 'minicart_nau_'+newCartProducts[i].productId+'_'+newCartProducts[i].skuId;
